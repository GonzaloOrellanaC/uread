...
const editOrganization = async (orgInfo: Organization, locale: string = env.locale): Promise<Organization> => {
    const org = await OrganizationModel.findByIdAndUpdate(orgInfo._id, orgInfo)
    if (!org)
        throw new HttpException(
            409,
            __({ phrase: 'error', locale }, { name: orgInfo.name })
        )
    return org
}
...
const getOrganizations = async (): Promise<Organization[]> => {
    const organizations = await OrganizationModel.find().populate('user')
    return organizations
}

const getOrgById = async (_id: string): Promise<Organization> => {
...
export default { createOrganization, deleteOrgById, getOrganizations, updateOrgById, getOrgById, editOrganization }
